...
type GlobalConfig = { email: string | null; name: string | null }

export function GlobalConfigEditor() {
  const [cfg, setCfg] = useState<GlobalConfig>({ email: null, name: null })
  ...
  async function save(partial: Partial<GlobalConfig>) {
    setSaving(true)
    setError(null)
    try {
...
type RepoConfigProps = { repoName: string }

export function RepoConfigEditor({ repoName }: RepoConfigProps) {
  ...
  async function save(partial: Partial<{ email: string; name: string }>) {
    setSaving(true)
    setError(null)
    try {
