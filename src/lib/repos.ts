import { Dirent, readdir, readFile, stat } from 'fs/promises'
...
export async function listLocalRepositories(rootDir: string = DEFAULT_ROOT): Promise<LocalRepository[]> {
  try {
...
export async function getLocalRepository(name: string, rootDir: string = DEFAULT_ROOT): Promise<LocalRepository | null> {
