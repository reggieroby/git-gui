...
export const Q_REMOTES = `query RepoRemotes($name:String!){ remotes(name:$name){ name url branches branchStatuses{ remote branch ahead behind } } }`
...
export const M_MERGE_BRANCH = `mutation MergeBranch($name:String!,$branch:String!,$strategy:String){ mergeBranch(name:$name, branch:$branch, strategy:$strategy){ ok fastForward error } }`
export const M_PUSH_BRANCH = `mutation PushBranch($name:String!,$branch:String!,$force:Boolean){ pushBranch(name:$name, branch:$branch, force:$force){ ok remote error } }`
export const M_REBASE_SQUASH = `mutation RebaseSquash($name:String!,$base:String!,$message:String){ rebaseSquash(name:$name, base:$base, message:$message){ ok base newHead count error } }`
