...
import { getLocalRepository, listLocalRepositories, listRepositoryRemotes } from '@/lib/repos'
...
async function chownRepoMaybe(repoPath) {
  const uid = process.env.UID
  const gid = process.env.GID
  if (!uid || !gid) return
  try {
    await execFile('chown', ['-R', `${uid}:${gid}`, repoPath])
  } catch (error) {
    console.warn('chown failed', error?.message)
  }
}
...
const RemoteBranchStatusType = new GraphQLObjectType({
  name: 'RemoteBranchStatus',
  fields: {
    remote: { type: new GraphQLNonNull(GraphQLString) },
    branch: { type: new GraphQLNonNull(GraphQLString) },
    ahead: { type: new GraphQLNonNull(GraphQLInt) },
    behind: { type: new GraphQLNonNull(GraphQLInt) }
  }
})
...
const RemoteType = new GraphQLObjectType({
  ...
  fields: {
    ...
    branchStatuses: { type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(RemoteBranchStatusType))) }
  }
})
...
const RepositoryType = new GraphQLObjectType({
  ...
  fields: {
    name: { type: new GraphQLNonNull(GraphQLString) },
    path: { type: new GraphQLNonNull(GraphQLString) },
    hasUnpushedCommits: { type: new GraphQLNonNull(GraphQLBoolean) }
  }
})
...
const MergeBranchResultType = new GraphQLObjectType({
  name: 'MergeBranchResult',
  fields: {
    ok: { type: new GraphQLNonNull(GraphQLBoolean) },
    fastForward: { type: GraphQLBoolean },
    error: { type: GraphQLString }
  }
})
const PushBranchResultType = new GraphQLObjectType({
  name: 'PushBranchResult',
  fields: {
    ok: { type: new GraphQLNonNull(GraphQLBoolean) },
...
const RebaseSquashResultType = new GraphQLObjectType({
  name: 'RebaseSquashResult',
  fields: {
    ok: { type: new GraphQLNonNull(GraphQLBoolean) },
    base: { type: GraphQLString },
    newHead: { type: GraphQLString },
...
const QueryType = new GraphQLObjectType({
  ...
  fields: {
    ...
    remoteBranchStatuses: {
      args: { name: { type: new GraphQLNonNull(GraphQLString) } },
      type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(RemoteBranchStatusType))),
      resolve: (_src, args) => remoteBranchStatuses(args)
    },
    status: {
      args: { name: { type: new GraphQLNonNull(GraphQLString) } },
...
const MutationType = new GraphQLObjectType({
  ...
  fields: {
    ...
    mergeBranch: {
      ...
      type: new GraphQLNonNull(MergeBranchResultType),
      resolve: (_src, args) => mergeBranch(args)
    },
    pushBranch: {
      args: {
        name: { type: new GraphQLNonNull(GraphQLString) },
    ...
    rebaseSquash: {
      ...
      type: new GraphQLNonNull(RebaseSquashResultType),
      resolve: (_src, args) => rebaseSquash(args)
    },
    stage: {
...
async function remotes({ name }) {
  const repoName = decodeURIComponent(name)
  const { repo, remotes } = await listRepositoryRemotes(repoName)
  if (!repo) throw new Error('Repository not found')
  return remotes
}

async function remoteBranchStatuses({ name }) {
  const repoName = decodeURIComponent(name)
  const { repo, remotes } = await listRepositoryRemotes(repoName)
  if (!repo) throw new Error('Repository not found')
  const result = []
  for (const remote of remotes) {
    if (remote.name === 'local') continue
    const list = Array.isArray(remote.branchStatuses) ? remote.branchStatuses : []
    for (const entry of list) {
      if (!entry || typeof entry.branch !== 'string') continue
      result.push({
        remote: entry.remote || remote.name,
        branch: entry.branch,
        ahead: typeof entry.ahead === 'number' ? entry.ahead : 0,
        behind: typeof entry.behind === 'number' ? entry.behind : 0
      })
    }
  }
  return result
...
async function mergeBranch({ name, branch, strategy }) {
  const repoName = decodeURIComponent(name)
  const repo = await getLocalRepository(repoName)
  if (!repo) throw new Error('Repository not found')
  if (!(await isGitRepository(repo.path))) throw new Error('Not a Git repository')
  ...
  try {
    ...
    const stderr = error?.stderr ? error.stderr.toString().trim() : null
    return { ok: false, fastForward: null, error: stderr || error?.message || 'Branch not found' }
  }
  const mergeArgs = ['-C', repo.path, 'merge']
  if (strategy === 'no-ff') mergeArgs.push('--no-ff', '--no-edit')
  else mergeArgs.push('--ff-only')
  mergeArgs.push(branchName)
  try {
    await execFile(gitBin, mergeArgs)
    await chownRepoMaybe(repo.path)
    const fastForward = strategy === 'no-ff' ? false : true
    return { ok: true, fastForward, error: null }
  } catch (error) {
    const stderr = error?.stderr ? error.stderr.toString().trim() : null
    return { ok: false, fastForward: null, error: stderr || error?.message || 'Failed to merge branch' }
  }
}

async function pushBranch({ name, branch, force }) {
  const repoName = decodeURIComponent(name)
  const repo = await getLocalRepository(repoName)
  if (!repo) throw new Error('Repository not found')
  if (!(await isGitRepository(repo.path))) throw new Error('Not a Git repository')
  ...
  try {
    ...
    const stderr = error?.stderr ? error.stderr.toString().trim() : null
    return { ok: false, remote: null, error: stderr || error?.message || 'Branch not found' }
  }
  let remoteName = null
  try {
    const { stdout } = await execFile(gitBin, ['-C', repo.path, 'config', '--get', `branch.${branchName}.remote`])
    remoteName = stdout.toString().trim()
  } catch {
    remoteName = null
  }
  if (!remoteName) remoteName = 'origin'
  if (!remoteName) return { ok: false, remote: null, error: 'No remote configured for this branch' }
  let mergeRef = null
  try {
    const { stdout } = await execFile(gitBin, ['-C', repo.path, 'config', '--get', `branch.${branchName}.merge`])
    mergeRef = stdout.toString().trim()
  } catch {
    mergeRef = null
  }
  const remoteRef = mergeRef && mergeRef.startsWith('refs/') ? mergeRef : `refs/heads/${branchName}`
  try {
    const args = ['-C', repo.path, 'push']
    if (force) args.push('--force-with-lease')
    args.push(remoteName, `${branchName}:${remoteRef}`)
    await execFile(gitBin, args)
    return { ok: true, remote: remoteName, error: null }
  } catch (error) {
    const stderr = error?.stderr ? error.stderr.toString().trim() : null
    return { ok: false, remote: remoteName, error: stderr || error?.message || 'Failed to push branch' }
  }
}
async function rebaseSquash({ name, base, message }) {
  const repoName = decodeURIComponent(name)
  const baseHash = decodeURIComponent(base)
  const repo = await getLocalRepository(repoName)
  if (!repo) throw new Error('Repository not found')
  if (!(await isGitRepository(repo.path))) throw new Error('Not a Git repository')
  const gitBin = process.env.GIT_BIN || 'git'
  if (!baseHash) throw new Error('Base commit required')

  const { stdout: originalHeadOut } = await execFile(gitBin, ['-C', repo.path, 'rev-parse', 'HEAD'])
  const originalHead = originalHeadOut.toString().trim()

  let commitList = []
  try {
    const { stdout } = await execFile(gitBin, ['-C', repo.path, 'rev-list', '--reverse', '--ancestry-path', `${baseHash}..HEAD`])
    commitList = stdout.toString().split('\n').map((s) => s.trim()).filter(Boolean)
  } catch (error) {
    const stderr = error?.stderr ? error.stderr.toString().trim() : null
    return { ok: false, base: baseHash, newHead: originalHead, count: 0, error: stderr || error?.message || 'Failed to enumerate commits' }
  }

  if (commitList.length === 0) {
    return { ok: true, base: baseHash, newHead: originalHead, count: 0, error: null }
  }

  const firstCommit = commitList[0]
  const restCommits = commitList.slice(1)

  let commitMessage = typeof message === 'string' ? message.trim() : ''

  if (!commitMessage) {
    let baseMessage = ''
    try {
      const { stdout } = await execFile(gitBin, ['-C', repo.path, 'show', '--quiet', '--pretty=format:%B', firstCommit])
      baseMessage = stdout.toString().trim()
    } catch { }
    if (!baseMessage) baseMessage = `Squash commits after ${baseHash.slice(0, 7)}`

    if (restCommits.length) {
      const summaries = await Promise.all(restCommits.map(async (commit) => {
        try {
          const { stdout } = await execFile(gitBin, ['-C', repo.path, 'show', '--quiet', '--pretty=format:%s', commit])
    ...
    commitMessage = baseMessage.trim()
  }

  if (!commitMessage) commitMessage = `Squash commits after ${baseHash.slice(0, 7)}`
  const subject = commitMessage.split('\n')[0].trim()

  try {
    await execFile(gitBin, ['-C', repo.path, 'reset', '--soft', baseHash])
    await execFile(gitBin, ['-C', repo.path, 'commit', '-m', subject])
    await chownRepoMaybe(repo.path)
    const { stdout: newHeadOut } = await execFile(gitBin, ['-C', repo.path, 'rev-parse', 'HEAD'])
    const newHead = newHeadOut.toString().trim()
    return { ok: true, base: baseHash, newHead, count: commitList.length, error: null }
  } catch (error) {
    try { await execFile(gitBin, ['-C', repo.path, 'reset', '--hard', originalHead]) } catch { }
    const stderr = error?.stderr ? error.stderr.toString().trim() : null
    return { ok: false, base: baseHash, newHead: originalHead, count: 0, error: stderr || error?.message || 'Failed to rebase and squash' }
  }
}
